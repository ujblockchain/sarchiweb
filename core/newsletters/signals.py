...
from core.bootcamps.models import BootcampSignup
...
def auto_mail_sending(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if group == 'Selected Bootcamp Group':
            # get the email list
            email_list = BootcampSignup.filter(application_status='Selected').values_list('email', flat=True)

        # for those not selected for bootcamps
        elif group == 'Rejected Bootcamp Group':
            # get the email list
            email_list = BootcampSignup.filter(application_status='Rejected').values_list('email', flat=True)

        # for those in coding session for bootcamps
        elif group == 'Bootcamp: Coding Session':
            # get the email list
            email_list = BootcampSignup.filter(
        ...
        # for those in drag and drop session for bootcamps
            ...
            # get the email list
            email_list = BootcampSignup.filter(
                session='No Coding Session (Drag and Drop Design)', application_status='Selected'
            ).values_list('email', flat=True)
        ...
        # init task
        send_email_task(
            sorted_email_list, instance.salutation, instance.subject, instance.message, instance.link,
            instance.link_title
        )
...
def auto_mail_reminder_sending(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        # init task
        reminder_email_task(
            sorted_email_list, instance.subject, instance.main_message_heading, instance.primary_message_section,
            instance.image_section_heading, image_1, instance.display_image_1_text, image_2,
            instance.display_image_2_text, image_3, instance.display_image_3_text, image_4,
